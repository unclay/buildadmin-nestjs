...
export class CoreClickCaptchaService {
  ...
  constructor(
    ...
    const clickConfig =
      this.configService.get<BuildAdminConfig>('buildadmin').click_captcha;
    this.config = {
      ...this.config,
      ...clickConfig,
    };
  ...
  async create(id: string): Promise<ClickCaptchaData> {
    ...
    const expireTime = new Date(Date.now() + this.config.expire * 1000);
    const key = crypto.createHash('md5').update(id).digest('hex');
    const code = crypto
      .createHash('md5')
      .update(textElements.join(','))
      .digest('hex');
  ...
  private generateRandomPoints(): ClickPoint[] {
    ...
    for (let i = 0; i < totalPoints; i++) {
      ...
      do {
        const randomChar = this.config.zhSet.charAt(
          Math.floor(Math.random() * this.config.zhSet.length),
        );
        point = {
