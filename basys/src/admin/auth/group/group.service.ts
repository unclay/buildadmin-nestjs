...
export class AuthGroupService extends CoreApiService {
  ...
  async getGroups(
    query: AuthGroupIndexQueryDto,
    where: Prisma.BaAdminGroupWhereInput = {},
  ) {
    ...
    // 权限处理
    if (!isSuperAdmin) {
      const authGroups = await this.coreAuthService.getAllAuthGroups(
        authMethod,
        where,
      );
      const groupIds = absoluteAuth
        ? authGroups
        : [...adminGroups, ...authGroups];
    ...
    // 组装树形结构
    return this.getAssembleTree(query)
      ? BaTree.getInstance().assembleChild(data)
      : data;
  }
  ...
  async getOptions(query: AuthGroupIndexQueryDto) {
    ...
    const list = await this.getGroups(query);
    return {
      options: list,
      query,
    };
      ...
      if (superAdmin && isSuperAdmin) {
        ...
        if (!array_diff(ownedRuleIds, checkedRules)) {
          throw ApiResponse.error(
            this.i18n.t(
              'auth.admin',
              'Role group has all your rights, please contact the upper administrator to add or do not need to add!',
            ),
          );
        }
        ...
        if (array_diff(checkedRules, ownedRuleIds) && !isSuperAdmin) {
          throw ApiResponse.error(
            this.i18n.t(
              'auth.admin',
              'The group permission node exceeds the range that can be allocated',
            ),
          );
        }
  ...
  private getAssembleTree(query: AuthGroupIndexQueryDto) {
    const { initValue = [], isTree = true } = query;
